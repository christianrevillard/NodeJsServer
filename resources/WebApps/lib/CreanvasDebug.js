...
(function() {
  ...
  creanvas.NodeJsController = function(controllerData) {
    ...
    this.emitToServer = function(action, actionData, overrideActionKey) {
      if (overrideActionKey && emitBuffer.length > 0) {
        ...
        if (last.actionData.eventId == actionData.eventId && last.overrideActionKey == overrideActionKey) {
          if (DEBUG) {
            this.logMessage("overrider non sent " + actionData.eventId + " for " + overrideActionKey + " to " + actionData);
          }
  ...
  var registerCanvasEvents = function() {
    this.registerCanvasPointerEvent("click", "click", this.triggerElementEvent);
    this.registerCanvasPointerEvent("mousedown", "pointerDown", this.triggerElementEvent);
    this.registerCanvasPointerEvent("touchstart", "pointerDown", this.triggerElementEvent);
    this.registerCanvasPointerEvent("mousemove", "pointerMove", this.triggerTouchIdentifierEvent);
    this.registerCanvasPointerEvent("touchmove", "pointerMove", this.triggerTouchIdentifierEvent);
    this.registerCanvasPointerEvent("mouseup", "pointerUp", this.triggerTouchIdentifierEvent);
    this.registerCanvasPointerEvent("touchend", "pointerUp", this.triggerTouchIdentifierEvent);
  };
  var addBackground = function(drawBackground, backgroundStyle) {
    var controller = this;
    if (DEBUG) {
  ...
  creanvas.NodeJsController.prototype.triggerElementEvent = function(eventId, event) {
    ...
    this.elements.sort(function(a, b) {
      ...
      if (element.hit(event.x, event.y)) {
        controller.emitToServer("elementEvent", {"eventId":eventId, "elementId":element.id, "touchIdentifier":event.touchIdentifier});
        hit = true;
      }
    });
  };
  creanvas.NodeJsController.prototype.triggerTouchIdentifierEvent = function(eventId, event) {
    var controller = this;
  ...
  creanvas.NodeJsController.prototype.registerCanvasPointerEvent = function(controlEventId, customEventId, trigger) {
    var controller = this;
    controller.context.canvas.addEventListener(controlEventId, function(event) {
      setTimeout(function() {
        var triggerEvent = function(clientXY, touchIdentifier) {
          ...
          eventData.touchIdentifier = touchIdentifier;
          trigger.call(controller, customEventId, eventData);
        };
        if (event.changedTouches) {
          for (var i = 0;i < event.changedTouches.length;i++) {
            triggerEvent(event.changedTouches[i], event.changedTouches[i].identifier);
