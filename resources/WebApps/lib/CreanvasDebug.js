...
(function() {
  ...
  creanvas.NodeJsController = function(controllerData) {
    ...
    setInterval(function() {
      ...
      emitBuffer.forEach(function(e) {
        if (DEBUG) {
          controller.logMessage("Emit to server: " + e.action + ", " + JSON.stringify(e.actionData));
        }
    ...
    this.nodeSocket.on("update", function(msg) {
      ...
      data.forEach(function(updated) {
        ...
        el.elementX = updated["x"] / controller.lengthScale || el.elementX;
        el.elementY = updated["y"] / controller.lengthScale || el.elementY;
        el.elementAngle = updated["angle"] || el.elementAngle;
      });
      needRedraw = true;
      (new CreJs.Crelog.Logger).logMessage("Upates:" + msg);
    });
    this.nodeSocket.on("addElement", function(msg) {
      var data = JSON.parse(msg);
      if (DEBUG) {
        controller.logMessage("Adding element " + data["drawingMethod"] + " in (" + data["x"] + "," + data["y"] + ")");
      }
      var element = controller.add(["name", data["name"]], ["image", {"left":data["left"], "top":data["top"], "width":data["width"], "height":data["height"], "draw":controller.drawingMethods.filter(function(e) {
        return e.drawingMethod == data["drawingMethod"];
      })[0].draw}], ["position", {"x":data.x, "y":data.y, "z":data.z, "angle":data.angle}]);
      element.id = data.id;
    });
  };
  var registerCanvasEvents = function() {
    this.registerCanvasPointerEvent("click", "click");
    this.registerCanvasPointerStartTouchEvent("mousedown", "pointerDown");
    this.registerCanvasPointerStartTouchEvent("touchstart", "pointerDown");
    this.registerCanvasPointerEvent("mousemove", "pointerMove");
    this.registerCanvasPointerEvent("touchmove", "pointerMove");
    this.registerCanvasPointerEndTouchEvent("mouseup", "pointerUp");
    this.registerCanvasPointerEndTouchEvent("touchend", "pointerUp");
  };
  var addBackground = function(drawBackground, backgroundStyle) {
    var controller = this;
    if (DEBUG) {
  ...
  creanvas.NodeJsController.prototype.triggerElementEvent = function(eventId, event, startTouch, endTouch) {
    var controller = this;
    var hitElement = null;
    ...
    this.elements.sort(function(a, b) {
      ...
      if (element.hit(event.x, event.y)) {
        hitElement = element;
        if (startTouch) {
          hitElement.touchIdentifier = event.touchIdentifier;
        }
      }
    });
    if (!startTouch) {
      var byIdentifier = this.elements.filter(function(e) {
        return e.touchIdentifier == event.touchIdentifier;
      });
      if (byIdentifier.length > 0) {
        touchElement = byIdentifier[0];
        if (endTouch) {
          touchElement.touchIdentifier = null;
        }
      }
    }
    var x = event.x;
    var y = event.y;
    if (hitElement) {
      controller.emitToServer("hitEvent" + hitElement.id, {"eventId":eventId, "x":x, "y":y, "touchElementId":touchElement ? touchElement.id : -1}, "hitEvent" + hitElement.id + ":" + eventId + ":" + (touchElement ? touchElement.id : -1));
    }
    if (touchElement) {
      controller.emitToServer("touchEvent" + touchElement.id, {"eventId":eventId, "x":x, "y":y, "hitElementId":hitElement ? hitElement.id : -1}, "touchEvent" + touchElement.id + ":" + eventId + ":" + (hitElement ? hitElement.id : -1));
    }
  };
  creanvas.NodeJsController.prototype.registerCanvasPointerStartTouchEvent = function(controlEventId, customEventId) {
    this.registerCanvasPointerEvent(controlEventId, customEventId, true, false);
  };
  creanvas.NodeJsController.prototype.registerCanvasPointerEndTouchEvent = function(controlEventId, customEventId) {
    this.registerCanvasPointerEvent(controlEventId, customEventId, false, true);
  };
  creanvas.NodeJsController.prototype.registerCanvasPointerEvent = function(controlEventId, customEventId, startTouch, endTouch) {
    var controller = this;
    controller.context.canvas.addEventListener(controlEventId, function(event) {
      setTimeout(function() {
        var triggerEvent = function(clientXY, touchIdentifier) {
          ...
          eventData.touchIdentifier = touchIdentifier;
          controller.triggerElementEvent.call(controller, customEventId, eventData, startTouch, endTouch);
        };
        if (event.changedTouches) {
          for (var i = 0;i < event.changedTouches.length;i++) {
            triggerEvent(event.changedTouches[i], event.changedTouches[i].identifier);
  ...
  creanvas.NodeJsController.prototype.getRealXYFromClientXY = function(clientXY) {
    ...
    var xy = {x:(clientXY.clientX - boundings.left) * this.context.canvas.width / boundings.width * this.lengthScale, y:(clientXY.clientY - boundings.top) * this.context.canvas.height / boundings.height * this.lengthScale};
    if (DEBUG) {
      this.logMessage("RealXY: (" + clientXY.clientX + "," + clientXY.clientY + ") - CanvasXY: (" + xy.x + "," + xy.y + ")");
    }
    return xy;
  };
  ...
  creanvas.NodeJsController.prototype.startApplication = function() {
    this.nodeSocket.emit("startApplication", "");
  };
